 ...
 *
 ---><cfcomponent extends="types.Driver" implements="types.IDatasource">
	...
	<li>
		...
		field("SID/Service name", "separator", ":,/",true,"Allow you to connect via SID (:) or SERVICE_NAME (/).","select",1)
	)>


	<cfset this.className="oracle.jdbc.OracleDriver">
	<cfset this.dsn="jdbc:oracle:{drivertype}:@{host}:{port}{separator}{database}">
